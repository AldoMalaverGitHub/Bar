...
class MainActivity : AppCompatActivity() {
    ...
    fun mostrar(view: View){
        var snack: Snackbar = Snackbar.make(
    ...
    override fun onOptionsItemSelected(item: MenuItem?):
        ...
        when(item?.itemId){
            R.id.item1 -> {Toast.makeText(this,
                    "Le ganamos 3-0", Toast.LENGTH_LONG)
                    .show()
            return super.onOptionsItemSelected(item)
            }

            R.id.item2 -> {Toast.makeText(this,
                    "Empatamos 1-1", Toast.LENGTH_LONG)
                    .show()
                return super.onOptionsItemSelected(item)
            }

            R.id.item3 -> {Toast.makeText(this,
                    "Le ganaremos 3-0", Toast.LENGTH_LONG)
                    .show()
                return super.onOptionsItemSelected(item)
            }

            R.id.item4 -> {Toast.makeText(this,
                    "Le ganamos 3-0", Toast.LENGTH_LONG)
                    .show()
                return super.onOptionsItemSelected(item)
            }
